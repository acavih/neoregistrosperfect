...
export default {
  components: { ResourceItemSelector, VuetifyDatePicker },
  data () {
    return {
  ...
  methods: {
    ...
    async createPartner () {
      try {
        ...
        await this.$axios.post('/api/partners', payload)
  ...
  <v-card tag="form" @submit.prevent="submitMember">
    ...
    <v-card-text class="mt-4">
      <v-row>
        ...
        <v-col cols="12" md="4">
          <vuetify-date-picker v-model="member.bornDate" label="Fecha de nacimiento" />
        </v-col>
        <v-col cols="12" md="4">
